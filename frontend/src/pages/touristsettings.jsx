...
const TouristSettings = () => {
    ...
    const fetchPreferences = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/getPreferences?username=${username}`
        );

        if (response.ok) {
          const data = await response.json();
          setPreferences(data.preferences);
          setUpdatedPreferences(data.preferences);
        } else {
          throw new Error("Failed to fetch preferences");
        }
      } catch (error) {
        setErrorMessage("Error fetching preferences");
      }
    };
  ...
  // UseEffect to fetch tourist info when the component mounts
  useEffect(() => {
    fetchTouristInfo();
  ...
  return (
          ...
          touristInfo && (
            <div>
              ...
              <div>
                ...
                <p>
                  {touristInfo.Wallet} {selectedCurrency}
                </p>
              </div>
    ...
    {Object.keys(preferences).length > 0 ? (
      <form>
        ...
        <div style={{ marginBottom: '20px' }}>
          <label style={{ fontWeight: 'bold' }}>Select Budget:</label>
          <select
            name="budget"
      ...
      <p>No preferences available.</p>
              ...
              <p>
                Price: {(touristInfo.Wallet * conversionRate).toFixed(2)}{" "}
                {selectedCurrency}
              </p>
