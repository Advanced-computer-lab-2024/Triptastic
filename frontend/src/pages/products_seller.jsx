...
const Productssel = () => {
  ...
  const archiveProduct = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/archiveProduct/${productNameToArchive}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok) {
        ...
        setSuccessMessage(
          `Product "${productNameToArchive}" archived successfully.`
        );
  ...
  const unarchiveProduct = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/unarchiveProduct/${productNameToArchive}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        const result = await response.json();
        setSuccessMessage(
          `Product "${productNameToArchive}" unarchived successfully.`
        );
